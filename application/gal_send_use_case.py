    ...
    def execute(
        ...
        *,
        ...
        try:
            ...
            _report("Passo 1/6: A iniciar o navegador Firefox...", 1 / total_steps)
            driver = self._webdriver_factory()

            _report("Passo 2/6: A realizar login no GAL...", 2 / total_steps)
            self._service.realizar_login(driver, request.usuario, request.senha)

            _report("Passo 3/6: A ler e validar o arquivo CSV...", 3 / total_steps)
            ...
            def _process_row(i_idx, row_data):
                ...
                duplicate_match_key = ""
                with lock:
                    if idempotency_key in successful_keys:
                        duplicate_match_key = idempotency_key
                    elif legacy_idempotency_key in successful_keys:
